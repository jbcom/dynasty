...
- [ ] **CP-R-ARCH-1 family-tree spines + slots for the 2 new archetypes.** Add family-trees/entertainment.json + athletic.json (generic spines), slots.dynasty.{entertainment,athletic} resolutions where a slot differs by power base. (The spine is only the pre-founding fallback now — live family drives founded runs — so these are thin.)
