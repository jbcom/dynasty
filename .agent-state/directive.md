...
- [ ] nb-006c remove dead `?? 0`/`?? 1` masking on MarketState reads in world.ts (lines ~88-93) + effects.ts (marketOps holding/leverage). MarketState fields are NON-OPTIONAL by type and guarded by `if(!ms)continue`/`if(!cur)continue`, so the fallbacks are unreachable and mask future real bugs. The Gemini PR #12 thread that prompted them ("legacy save-state market fields") is based on a WRONG premise: saves are seed+history only (save.ts toSave/fromSave), market state is ALWAYS reconstructed by initState→initMarkets on replay — no save ever carries a partial MarketState. Answer the thread with this architecture rather than masking. Keep the legitimate koota `.get(Component)?.x ?? d` fallbacks (koota getters genuinely can be undefined).
