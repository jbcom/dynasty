...
- [ ] **CP-7r-a foundByComposition (sim).** Generalize foundDynasty to a composed origin {place,era,culture,gender,surname?,calling?,axisChoices?}; birthYear from era; culture default from place; keep momentId as a thin convenience. Save carries the composition; replay parity. Tests.
- [ ] **CP-7r-b place/era resolution data.** places catalog (sensory cue → place) + per-place valid era windows + default culture; cross-ref every (place×era) → world-stack + culture + real era.
- [ ] **CP-7r-c diegetic birth event sequence.** Authored 6-slot emergence events (sensory→place, son/daughter/other→gender, name bestowal) + the era beat, as engine events flagged epoch0/birth, gated to run first. New Game drops straight in.
- [ ] **CP-7r-d retire the panel UI.** TitleScreen → title (New Game · Load · Settings) only; New Game starts the birth sequence in PlayScreen. Remove the carousel/name/calling/axes panel + tests.
- [ ] **CP-7r-e player-facing copy.** No internal identity nouns in shown text; the founded line named in its own terms.
- [ ] **CP-8 Content breadth.** Author the lived calling + axis Epoch-0 beats per place/era (Gemini toolkit for breadth); branch-density still passes.
- [ ] **CP-9 DoD.** Full gate + AH6/persona/founded-longrun sweeps over the new model + live-verify the birth→Epoch-0 per a modern AND the deep-history line; PR; reviewer trio; squash-merge; Status RELEASED.
