...
- [~] **CP-R6 partial: diegetic calling beat done (setsCalling + ev_birth_calling).** Axis-choice diegetic beats fold into the harness review (CP-R7) — author/verify them against the dumped timeline rather than blind.
- [x] **CP-R7 DONE: dev harness + bespoke→slot dissolution to 0 leaks + DoD.** Built the harness (src/sim/harness.ts: tracePlaythrough/validateTrace/auditTimelines) + the in-app DEV overlay (fast-forward +1/+10/+100 + timeline JSON dump). The audit caught 638 preset-person leaks; refactored EVERY bespoke literal-bio event into generic slotted archetype/trope content (Fred→your father, Friedrich→the founder, Kennedy/Musk prologues → political/technological founder tropes archetype-gated, rival-house timelines → the tech magnate / the rival dynasty / the great evangelist). Result: 0 findings over 180 Gen-0 permutations / 7058 beats (artifacts/timeline-audit.json); 0 preset-person literals in any player-facing field across all content. Live-verified modern (east_coast/bavaria/south_africa) AND deep-history (baghdad/caliphate) lines render their own name, 0 leaks. Docs aligned (README + new docs/STATE.md + CLAUDE.md). e2e rewritten for the diegetic flow + run against the preview build. FULL GAUNTLET GREEN: typecheck 0/0, 474 unit, 67 browser, 7 e2e, build OK. **PR #31 open (jbcom/dynasty).**
- [ ] [WAIT] PR #31 CI green + reviewer threads (CodeRabbit/Q/CodeQL). Then address feedback, resolve threads, squash-merge, flip Status → RELEASED, compress CP-R* → directive-archive.md. Next branch = pure expansion (1000-year dynasty, max permutations) per §NEXT BRANCH.

### NEXT BRANCH (user, 2026-06-21) — queued for immediately after THIS branch squash-merges
After CP-R7's PR is merged: **immediate pivot + directive compression to full exploration +
construction of all the GenAI-discussed pieces**, to fulfill that branch's ultimate goal — a
**dev-mode 1000-YEAR dynasty** with as many permutations as possible, filling out tons of
different possibilities across the ages. (The harness from CP-R7 — trace/dump/validate +
the dev overlay — is the substrate; the next branch drives GenAI breadth to populate the
place×era×archetype×branch space so a single line can run a full millennium richly.) Compress
the completed CP-R* queue into directive-archive.md and open the new milestone fresh.
