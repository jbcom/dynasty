...
- [ ] nb-006 balance + content polish. DONE (commit 3c4fe57): marketOps wire market holdings to choices (markets were inert) + Manhattan op + unit test; heat passive decay (-1.5/yr, floored at 0) + test; capped pol_win_accept_mandate power outlier 30->18; throttled the 1-click end_early_obscurity early-out (minEraOrder 3, minAge 30). DONE (this commit): the "~24% no-downside choices" premise was a FALSE POSITIVE — the scan treated `heat` (a NEGATIVE/scrutiny meter) as upside and ignored that flag-fork choices are real branch decisions, not meter gambles. Heat-aware, only 13 events are all-upside and ALL 13 are branch-forks (apex victory laps / personality forks), not fake tension. Correct fix applied: added a felt heat-cost to 7 domineering apex choices in redplanet so domination isn't free, instead of deleting legit safe options. REMAINING (agents in flight): historical date nits, ending epitaph enrichment, gap events (Epstein / heirs / 2000 Reform) — dispatched to date-fixer/ending-writer/gap-events, verify-before-fold pending.
