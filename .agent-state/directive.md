...
- [ ] **CP-R-AUDIT Exhaustive bespoke-content audit (user, 2026-06-20).** Because there is SO much bespoke Trump (and Musk, Kennedy, and all the alt-history branch) content, go through ALL of it and classify every event by what it really is: (a) power archetype(s) it serves, (b) the place it belongs to, (c) the time/period, (d) whether its "branch" (nazi/theocracy/media/…) is really a place-and/or-time-slotted dynasty rather than a bespoke literal. Output a full re-slotting map (event id → place × period × archetype(s) × branch-as-condition). This is the input to CP-R-ARCH redistribution. No literal-bespoke content survives un-slotted — even the Nazi-era content becomes generic content gated by a place/time/condition.
- [ ] **CP-R-ARCH Split the conflated power archetypes; redistribute ALL bespoke content (user, 2026-06-20).** Add `entertainment` + `athletic` as first-class archetypes (6 total) alongside economic/political/technological/religious. Each gets a family-tree spine, ARCHETYPE_START + SEED_FLAGS, slots.dynasty entry, content fan-out. Per the CP-R-AUDIT map: re-slot EVERY bespoke event (Trump life-arc, Musk/Kennedy prologue chains, the alt-history branch timelines) into place × period × archetype, with branch (nazi/theocracy/…) re-expressed as a place/time/flag CONDITION, not a bespoke file. Multi-archetype events tagged for each + gated at runtime (absorb & redistribute, NO duplication). Events declare `archetypes: [...]`; pool filters by run archetype (composes with place/branch gating + CP-R-ERA). Spec §CP-R-ARCH. Gate green. (Feeds CP-R6.)
