...
- [ ] FD-3.2 run `pnpm retag-tropes` over all authored eras → review proposed trope tags → commit the trope:<id> tags onto authored events (the literal lines become trope-flavored content).
- [ ] FD-3.3 remove orphaned flip/swap DATA + tests: role-flip endings (end_role_flip_tycoon, end_reich_industrialist), ev_flip_* events (interregnum/victory), kennedy_swap, trump_commercial_path/musk_political/kingmaker_mogul flags; delete role-swap memory; retire worldtime.ts linking after live parity-verify. Preset trees become trope-flavored quick-starts. No house-leak possible.
