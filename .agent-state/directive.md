...
- [x] de-1b DONE: added queryRunContext (branch+pole off the projected world — gives the previously-consumerless Branch/Pole traits a real reader; parity = branchOf/moralPoleOf) + queryEligibleForPole (eligible set tagged with the run pole — the ECS-native grouping the DE-2 HUD + DE-6 persona analytics consume). DECISION: did NOT force branchOf/moralPoleOf/timelinesForBranch/applyWorldFlags themselves through the ECS — they are run-scalar resolutions + list ops, not per-entity groupings, so a projection would be the wrong shape and add noise. The right de-1b move was to give the existing projection real consumers, not to launder pure scalars through a world. 265 tests green.
- [x] de-1c PHASE BOUNDARY gate GREEN: typecheck 0 errors, biome clean, 265 unit + 59 browser tests, timeline:sweep all 7 branches ok (replay/consistency). Opening PR #A "koota query substrate". (reviewer trio + thread resolution + squash-merge on PR open below.)
...
- [ ] de-4a Era-0 prologue choice for sibling-count + birth-order setting flags (only_child/firstborn_heir/fourth_child/fred_jr_present/fred_jr_died) that drive given-name resolution (AH8c → Friedrich III when firstborn/heir), inheritance logic, and the groomed-vs-accidental-heir framing across the compiled timeline. Tests for each lever's downstream effect. PHASE BOUNDARY PR #D.

### Phase DE-5 — THE DYNASTY PRODUCT EPIC (rename + 3 playable sagas + carousel)
- [ ] de-5a RENAME the product to "DYNASTY" — app title, package metadata, in-app branding, docs (README/CLAUDE.md/STANDARDS), capacitor app name/id. NOT the git repo slug unless trivially safe; keep release-please happy. The systems are a generic dynastic-saga grammar; reframe copy accordingly.
