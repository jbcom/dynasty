...
PIVOT (user, 2026-06-20): from "pick 1 of 4 fixed houses" → "FOUND YOUR OWN line."
Stage 0 recast: the player chooses NAME + WHEN + WHERE at a historically pivotal
moment (Irish Famine, Bavarian emigration, post-apartheid SA, …); the run is the
bespoke compiled JSON storyline for SEED + STAGE-0. Choices BEGET CHILDREN; the
family tree GROWS; add BIRTH / DEATH / INHERITANCE / ESTATE-PLANNING / SUCCESSION
mechanics across generations. Player-NAMED but real history/places kept; freed
from real-name constraints AND from portraits. The 4 composite archetypes survive
as OPTIONAL QUICK-START PRESETS on this engine. Full design:
...
- [x] FD-2.1 DONE (PR #27, merged): EventSchema historicity (real|extrapolated|personal) + place; historicityOf() reconciles legacy extrapolated. Backward-compatible.
- [x] FD-2.2 DONE (commit 38010c1): projectWorldEvents (src/sim/worldEvents.ts) converts all 1169 world-timeline entries → unified year-keyed REACTABLE GameEvents (headline→title, body→scene, historicity real|extrapolated, place from scope, bias.branch from timeline branch, setFlags preserved on a "Live through it" choice). Content.worldEvents holds the year-sorted pool. NO-LEAK tags: musk/kennedy scopes → dynasty:<id> (gate enforced in FD-2.3 selector).
- [x] FD-2.3 DONE: eligibleEvents weaves the era's protagonist beats with the projected world-events, EXCLUDING other-dynasty-owned ones (no-leak gate, ownedByOtherDynasty). World-events are AMBIENT PUNCTUATION: windowed to the current year (±12y) + capped (≤4 offered), low weight (1), and CLOCK- + BUDGET-NEUTRAL in applyChoice (era-`__world__` events don't advance lastEventYear or eraEventCount) so the family's ~12-16 life beats stay the spine. 358 tests green incl. prologue reachability + chronology + de6b persona sweep; 7-branch sweep consistent. (worldtime.ts linking still present in parallel; retire after live parity-verify — folded into FD-3.)
...
- [x] FD-3.1 DONE: canonical trope catalog as data + schema + cross-ref gate + retag toolkit job. src/data/tropes.json = the full 20-trope catalog (4 spine + branch + the previously-missing set), single source of truth. TropeSchema/TropeKindSchema/TropesFileSchema in schema.ts; buildContent parses tropes + enforces every `trope:<id>` event tag resolves to a catalog id (guarded on non-empty catalog so legacy fixtures load). loadContent wires tropes.json. scripts/extrapolate refactored to READ the catalog from tropes.json (prompt.mjs loadTropes — no more hardcoded list drift) + new trope-retag job: retagEvent (gemini.mjs, PRO+thinking), buildRetagPrompt, RETAG_SCHEMA, `pnpm retag-tropes` (dry by default, --write applies validated trope tags; --era/--only-untagged). 6 new trope tests; 364 green; typecheck + biome clean.
- [x] FD-3.2 DONE (commit 08796da): retag ran over all 13 era pools — 272 trope:<id> tags applied; every tag catalog-valid (cross-ref gate accepts all); 391 green.
- [x] FD-3.3 DONE (non-era-JSON 4d5e0fb; era-JSON f2f2696): removed all 8 ev_flip_* events (interregnum/victory), scrubbed dead swap flags (kennedy_swap/role_flip/trump_commercial_path/musk_political_turn/kingmaker_mogul) from era + timeline setFlags/clearFlags/requires/tags, updated timelines test to assert no-leak. FD-3 (tropes-as-influences) COMPLETE. worldtime.ts linking retirement deferred to its own item (needs live parity-verify) — see FD-3.4. Original detail: deleted role-flip endings (end_role_flip_tycoon, end_reich_industrialist) from endings.json + their 2 endings tests; removed dead `kennedy_swap` clause from compiler.dynastyOf + updated compiler/ah6-sweep tests; deleted role-swap memory + fixed timelines-architecture memory. REMAINING (after retag, era JSONs): ev_flip_* events (interregnum ×4: ev_flip_natural_born_repeal/musk_ascends/tycoon_reborn/kingmaker_play; victory ×5: ev_flip_musk_takes_power/commercial_vs_political/mars_money/base_belongs_to_whom) + kennedy_swap flag (origins.json:303) + trump_commercial_path/musk_political/kingmaker_mogul flags; update timelines.unit.test.ts swap-describe block; review swap flags in timelines/{kennedy,musk}.json; retire worldtime.ts linking after live parity-verify. No house-leak possible.
### FD-4 PROCEDURAL POOL (user — 100k events / 1000 yrs; spec §1d)
- [x] FD-4.1 DONE (commit e646ec0): expander CORE. DECISION (spec §1d.1): purpose-built pure seeded grammar, NOT tracery (Math.random ban + needs structured GameEvent output + typed seeded slot resolution). schema: EventTemplate/TemplateChoice/TemplateSlot/Range/EventTemplatesFile. src/sim/procgen/ barrel: expandTemplate(template,ctx,rng) → concrete GameEvent (slots {member}{rival}{place}{year}{peril}{trope}{surname}, ranged deltas drawn via rng.fork, trope:<id>+procedural tags, validates against real EventSchema, deterministic=replay-safe-no-persistence). 8 colocated tests.
- [x] FD-4.2 DONE (commit after e646ec0): lazy bounded materialization. content.templates (EventTemplatesFile) cross-ref-validated against catalog; loadContent globs templates/*.json. procgen/context.buildExpandContext (member/rival/surname from active dynasty tree, seeded in-window year, per-dynasty place + era-generic perils = the FD-5/FD-7 seam). procgen/materialize.materializeProcedural (seeded Fisher–Yates order, cap-bounded, trope-label for {trope}). events.eligibleEvents gained optional rng: injects ≤PROC_CAP procedural events when authored forward pool < PROC_THRESHOLD (rng-less analysis path stays pure-authored). starter templates/dynastic.json. 7 tests; 381 green.
- [ ] [WAIT] FD-4.3 deep-history breadth exemplars + bulk — BLOCKED on prerequisites: deep-history templates need deep-history eras (FD-6 start-moments) to attach to; Gemini bulk long-tail needs FD-3.2 retag finished (waiter bzx5raar1). Re-sequenced AFTER FD-6. (Authoring templates for nonexistent eras now would be untestable scaffolding — violates build-only-what-the-step-needs.)
...
- [x] de-ui-d DONE — PR #19 (DE-1c/DE-2c/DE-3/DE-UI) squash-merged to main; gate green, live-verified.
...
- [x] de-5a DONE: DISPLAY RENAME done (commit 7f6fb88) + docs copy reframe: README/CLAUDE.md/STANDARDS reframed from "MAGA Money Moves" → "Dynasty" + generic dynastic-saga framing; assets.json attribution strings updated.
- [x] de-5b DONE: Musk playable Era-0 saga — ev_dynasty_founding_choice (year 1885) gates Trump vs Musk path; 3 Musk prologue events (ev_walter_musk_cape_colony 1906, ev_errol_musk_builds 1940, ev_elon_musk_born 1946) all within origins era bounds; ev_donald_is_born patched with musk_dynasty_active exclusion; 11 new de-5b tests (8 chain-gate + 2 guard invariants + 1 compiler); 326 tests green. PR #21 → jbcom/dynasty/pull/21.
- [x] de-5c DONE: Kennedy playable Era-0 saga — ev_dynasty_founding_choice gets 3rd choice (choose_kennedy_dynasty); Patrick Kennedy Irish Famine chain (1886→1895→1920) with 3-choice events ev_patrick_kennedy_famine/ev_pj_kennedy_ward_boss/ev_jpk_the_patriarch; guards patched on ev_donald_is_born/ev_walter_musk_cape_colony/ev_brewing_prohibition_crossroads; 8 new de-5c tests + 1 compiler test; 339 tests green. commit 70bfeec on de-5b-musk-saga branch.
- [x] de-5d DONE: DYNASTY-SELECT CAROUSEL — TitleScreen step machine (title → dynasty-select), 3 dynasty cards (Trump/Musk/Kennedy) with house accents + founding/tagline copy, back button, chooseDynasty → onNewGame(seed, dynasty) → GameStore → initState(content, seed, dynasty). Full stack wired. 5 new browser tests (carousel shows 3 dynasties, Trump card fires "trump", Musk card fires "musk", back returns to title, existing seed test updated for 2-step flow); e2e updated with startGame(page, {seed, dynasty}) helper + dynasty-carousel e2e test + Musk-route e2e test. 339 unit + 67 browser green.
...
- [x] de-6a DONE (PR #22, merged to main; serial — NO agent fan-out per user directive): ah6-sweep.unit.test.ts is a 12-permutation CI-enforced consistency harness over Era-0 permutations × dynasties, asserting branch/title/dynasty coherence + no title-leaks (e.g. "President" in a Nazi run) + timeline-variant correctness. Repeatable test harness, not a one-off agent run. + de-6a-consistency-sweep-report.md. Also folded the 4 DE-1/DE-2 reviewer findings (PR #23, merged) + the ah6 type-assertion thread.
### nb-fix Playability fixes (user reports during play, 2026-06-20)
- [x] nb-fix DONE — PR #24 squash-merged: Epoch-0 prologue-skip fixed (birth gated on dynasty_capital + budget 12→16 + regression test) and cartoon portraits removed (component/field/412 refs/15 assets); live-verified per dynasty, 0 console errors.
