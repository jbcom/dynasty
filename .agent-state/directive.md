...
- [ ] [WAIT] nb-004 — persona-1 (optimizer/speedrunner) DONE; 2 agents still running. VERIFIED findings to action in nb-004b (real): (a) MARKETS INERT — MarketState.holding starts 0 and NO choice ever sets it, so all 6 markets deliver zero meter delta (the SIM1 market layer has no gameplay effect yet — needs choices that set/add holding, e.g. mogul real-estate, brand licensing); (b) 1-CLICK SPEEDRUN — ev_fred_brings_home_lessons/play_with_toys sets walked_away → end_early_obscurity fires turn 1 (needs a min-era/age throttle); (c) power=30 outlier on brand_pol_wins_1992/pol_win_accept_mandate (2x ceiling — cap or add heat); (d) heat has NO passive decay (one-way accumulator); (e) 23.7% of choices have zero downside (too little tension). DEBUNKED (agent false-positives from an autoPlaythrough probe that skips the world-flag broadcast + resolveRoles): the "7 dead gate flags / 3 unreachable endings" — 8/9 flags ARE set by world-timeline events (musk/science/usa/manhattan/religion) delivered via the linking broadcast, and role_flip is correctly derived by resolveRoles from musk_takes_power (verified). fan out persona agents over compiled-timeline dumps (timeline:dump) to find dead-ends/shallowness/dominant strategies AND missing branches; aggregate → fix → re-sweep.
