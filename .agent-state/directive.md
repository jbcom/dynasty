...
- [x] **CP-7 DONE (live-verified):** control-panel UI. TitleScreen rebuilt as a step machine: title → moment picker → naming (surname + Patriarch/Matriarch toggle + seeded given-name with reroll) → calling picker (the 6 callings) → Epoch-0 axes ("The First Convictions": each axis shows its place-scaled intensity "charged/present/muted here") → confirm → onFound(full FoundingInput). App.foundGame takes the full config; passes content callings/axes/worldStacks/onomastics. Browser + visual + e2e tests rewritten (471 unit + 68 browser + 8 e2e green). LIVE-VERIFIED (chrome-devtools): walked Baghdad line — naming suggests "Yahya" (Arabic, bug fixed), Calling "The Family's Bent" renders 6, Epoch-0 shows Faith CHARGED HERE for Baghdad; zero console errors. (Partner beat is an in-game Epoch-0 event via takesPartner, not a founding-panel step — content in CP-8.)
