...
### nb-006b Chronology tests + corrections (this session)
- [x] nb-006b LINEAR-TIME test pair shipped (commit 270eff5): chronology-data (sacred timeline — era monotonicity, event-in-era-bounds, world-timeline total order) + chronology-runtime (10 seeded playthroughs + jump/advance/replay all prove year/eraIndex/age/lastEventYear non-decreasing). Caught+fixed: 4 events authored outside era bounds; initState seeded age:0 at year 1885 (now ageInYear-derived, monotonic); 3 world-timeline files reordered chronologically. 261 tests green.
- [x] nb-006b CORRECTION: kennedy.json wk2_jpk_bank_president reverted 1913->1914 (JPK became Columbia Trust president in 1914 at age 25; the agent's 1914->1913 change was a regression, not a fix).
### nb-006c Cleanup queued (do AFTER agents release files — currently racing rev-code/gap-events on world.ts/effects.ts)
- [ ] [WAIT] nb-006c (BLOCKED on active agents rev-code/gap-events holding world.ts/effects.ts — two of my edits to these files were clobbered mid-flight; proceed once they finish). remove dead `?? 0`/`?? 1` masking on MarketState reads in world.ts (lines ~88-93) + effects.ts (marketOps holding/leverage). MarketState fields are NON-OPTIONAL by type and guarded by `if(!ms)continue`/`if(!cur)continue`, so the fallbacks are unreachable and mask future real bugs. The Gemini PR #12 thread that prompted them ("legacy save-state market fields") is based on a WRONG premise: saves are seed+history only (save.ts toSave/fromSave), market state is ALWAYS reconstructed by initState→initMarkets on replay — no save ever carries a partial MarketState. Answer the thread with this architecture rather than masking. Keep the legitimate koota `.get(Component)?.x ?? d` fallbacks (koota getters genuinely can be undefined).
### nb-005 Definition of done
- [ ] [WAIT] nb-005 DoD — PR #12; CI green (build-and-test pass on prior run); security+code-reviewer clean; app live-verified (1885→1905 playthrough, meters+market ticking); Gemini threads: re-answer the null-coalescing thread per nb-006c (premise wrong) instead of masking; biome forEach lint fix pushed (37ae272); awaiting CI re-run → squash-merge.
