...
- [ ] [WAIT] **FD-11 STRICT regeneration running** (waiter bxntptzgt). First pass (105 events) FAILED the branch-density quality gate: Gemini defaulted to 2-choice, ungated events, diluting 5 eras below the >0.6-multi-choice / 25%-gated bars. FIX: reverted all 105; tightened the generator — schema minItems 3 + a `requires` gate field; prompt demands 3-4 choices + ~1/3 flag-gated for path-dependence. Re-running `--write --count 8`. After: normalize-events.mjs → branch-density gate must PASS → typecheck → full suite → commit bulk content + FD-12/13/14. Then iterate to the 1000-yr no-dead-ends ship bar (persona/AH6 multi-gen sweep).
...
- [ ] [WAIT] **FD-15 DoD (whole game)** — GATED on FD-11 generation finishing (waiter bgsjc8drw) + the batched FD-11/12/13/14 commit. Then: normalize → full gate (typecheck + biome + unit + browser + e2e) + AH6 + persona sweeps over MULTI-GENERATION runs + 1000-year determinism stress; app live-verified end-to-end (found → beget → die → succeed → carry forward); open ONE PR; reviewer trio; resolve threads; squash-merge green; directive → RELEASED. Next branch = use + refine the game + GenAI interactions.
